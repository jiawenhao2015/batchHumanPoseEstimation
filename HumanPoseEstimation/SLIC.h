...
public:
	...
	//===========================================================================
	///	Get the result image with contours on the given color  加了一个参数 为了传递回人体种子点 
	//===========================================================================
	cv::Mat GetImgWithContours(cv::Scalar color,Mat& slicBody);

	int*		label; // label record the superpixel pixel belongs to 被我改成了公有的
	vector<vector<int>>pixelBelongToSeedsX;///保存属于每一个种子点的像素位置
	vector<vector<int>>pixelBelongToSeedsY;///二维数组 每一行代表属于一个种子点的所有像素

	string outputPath;//用来保存临时结果定义的变量	
...
private:
	...
	void DrawSeedsOnBody(cv::Mat& image, Mat& slicBody);
