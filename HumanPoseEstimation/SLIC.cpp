	...
	const int&					NUMITR)
{
	...
	while (numitr < NUMITR)
	{
		...
		for (int n = 0; n < numk; n++)   //所有种子点
		{
			int y1 = std::max(0, (int)(kseedsy[n] - offset));//上
			int y2 = std::min(m_height, (int)(kseedsy[n] + offset));//下
			int x1 = std::max(0, (int)(kseedsx[n] - offset));//左
			int x2 = std::min(m_width, (int)(kseedsx[n] + offset));//右

			for (int y = y1; y < y2; y++)        ///每个种子点周围  计算像素到种子点的距离 并记录
			{
				for (int x = x1; x < x2; x++)
				{
					int i = y*m_width + x;
	...
	const string&				path)
{
	...
	//_splitpath_s(filename.c_str(), NULL, 0,NULL,0, fname,0, extn);
	string temp = fname;
...
#pragma omp for
	...
	///保存属于每一个种子点的像素位置 二维数组 每一行代表属于一个种子点的所有像素
	vector<vector<int>>labelBelongToSeedsX(numlabels,vector<int>(0));
	vector<vector<int>>labelBelongToSeedsY(numlabels, vector<int>(0));
	//ofstream outfile;
	//char tempPath[128];
	//sprintf_s(tempPath, "%s/label.txt", outputPath.c_str());
	//outfile.open(tempPath);
	for (int i = 0; i < HEIGHT; i++)
	{
		for (int j = 0; j < WIDTH; j++)
		{
			//outfile << label[i * WIDTH + j] << " ";
			labelBelongToSeedsX[label[i * WIDTH + j]].push_back(j);//X
			labelBelongToSeedsY[label[i * WIDTH + j]].push_back(i);//Y
		}
		//outfile << endl;
	}
	//outfile.close();

	this->pixelBelongToSeedsX = labelBelongToSeedsX;
	this->pixelBelongToSeedsY = labelBelongToSeedsY;
...
void SLIC::GenerateSuperpixels(cv::Mat& img, UINT numSuperpixels, Mat depthData)
{
	if (img.empty()) {
		exit(-1);
	}
...
//遍历处理后的人体深度图 记录像素位置 x y 点云位置 Tx Ty Tz	
//这个函数核心部分与下边DrawSeedsOnBody 核心部分是否可以放在一起
void SLIC::SaveBodyPoints(Mat body)
{	
	//文件是否存在 存在就删除 然后重新建
	//if ((_access("output/slic_body.txt", 0)) != -1)
	//{
	//	printf_s("File slic_body.txt exists.\n");
	//	if (remove("output/slic_body.txt"))
	//		printf("Could not delete the file &s \n", "1.txt");
	//}
	ofstream fout;
	char tempPath[128];
	sprintf_s(tempPath, "%s/slic_body.txt", outputPath.c_str());
	fout.open(tempPath);
	//保证slic_body.txt中的数据格式与output0.txt 一样 按列遍历
	for (int j = 0; j < m_width; j++)
	{
		for (int i = 0; i < m_height; i++)
		{
			if (body.at<Vec3b>(i, j)[0] == 0)//第一个分量不为255 不为蓝色
			{
				fout << j << " " << i << " "
					<< originalDepth.at<Vec3d>(i, j)[0] << " "
					<< originalDepth.at<Vec3d>(i, j)[1] << " "
					<< originalDepth.at<Vec3d>(i, j)[2] << endl;
			}
		}
	}
	fout.close();
...
//给出经过超像素处理之后的人体点
void SLIC::DrawSeedsOnBody(cv::Mat& image, Mat& slicBody)
{
	...
	for (int i = 0; i < m_height; i++)
	{
		for (int j = 0; j < m_width; j++)
		{
			body.at<Vec3b>(i, j)[0] = 255;//blue
			body.at<Vec3b>(i, j)[1] = 0;
			body.at<Vec3b>(i, j)[2] = 0;
		}
	}
	for (int i = 0; i < kseedsx.size(); i++)
	{
		...
		for (int x = -1; x <= 1; x++)
		{
			...
			{	//八邻域有白色 认为当前无效
				...
				if (kseedsy[i] + y < 0 || kseedsy[i] + y >= height || kseedsx[i] + x<0 || kseedsx[i] + x>=width)continue;
		...
		if (flag)
		{
			...
			circle(image, position, 2, Scalar(0, 0, 0), -1);//种子点显示
			bodyparts++;

			//画一个circle 会影响旁边像素值
			//circle(body, position, 2, Scalar(0, 0, 255), -1);
			body.at<Vec3b>((int)kseedsy[i], (int)kseedsx[i])[0] = 0;
			body.at<Vec3b>((int)kseedsy[i], (int)kseedsx[i])[1] = 0;
			body.at<Vec3b>((int)kseedsy[i], (int)kseedsx[i])[2] = 255;
		}
	}
	...
	//waitKey(1);
	char tempPath[128];
	sprintf_s(tempPath, "%s/body.jpg", outputPath.c_str());
	imwrite(tempPath, body);

	SaveBodyPoints(body);

	//ofstream outfile;	
	//outfile.open("output/label.txt");	
	//for (int i = 0; i < 480;i++)
	//{
	//	for (int j = 0; j < 640;j++)
	//	{
	//		outfile << label[i*640+j] << " ";
	//	}
	//	outfile << endl;
	//}
	//outfile.close();
//SaveSuperpixelLabels(GetLabel(), 640, 480, , "output");
}

cv::Mat SLIC::GetImgWithContours(cv::Scalar color,  Mat& slicBody)
{
	if (type == GRAY) {
		...
		//DrawContoursAroundSegments(bufferRGB, label, m_width, m_height, color);//注释后没有轮廓
		cv::Mat result(m_height, m_width, CV_8UC4);
		memcpy(result.data, bufferRGB, m_width*m_height*sizeof(UINT));
		cvtColor(result, result, CV_BGRA2BGR);

		DrawSeedsOnBody(result,slicBody);		
