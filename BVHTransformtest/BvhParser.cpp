    ...
    void BvhParser::run(BvhData *bvhData, const BvhLexingData *lexingData)
    {
        ...
        while (index < lexemesSize)
        {
            ...
            case E_BVH_TOKEN_NT_KEYWORD_POSITION_LIST:
                // Xposition, Yposition, Zposition, Xrotation, Yrotation, Zrotation, Xscale, Yscale, Zscale
                if (lexeme.LexemeType == E_BVH_LEXEME_TYPE_KEYWORD &&
                    lexeme.getKeywordValue() >= E_BVH_TOKEN_TKW_XPOSITION && lexeme.getKeywordValue() <= E_BVH_TOKEN_TKW_ZSCALE)//lexeme.getKeywordValue() <= E_BVH_TOKEN_TKW_ZROTATION)
                {
                    // <KEYWORD_POSITION_LIST> --> <KEYWORD_POSITION> <KEYWORD_POSITION_LIST>
                    parsingStack.push(E_BVH_TOKEN_NT_KEYWORD_POSITION);
					bvhHierarchyNodeStack.top().channels[channelIndex++] = lexeme.getKeywordValue();
					//////XPOSITION YPOSITION ZPOSITION == 0
					////if (lexeme.getKeywordValue() == E_BVH_TOKEN_TKW_XPOSITION)
					////{
					////	if (abs(lexeme.getFloatValue())>1.0e-6)
					////	{
					////		std::cout<<lexeme.getFloatValue()<<std::endl;
					////	}
					////	
					////}
            ...
            case E_BVH_TOKEN_T_FLOAT_OFFSET:
                // float
				if (lexeme.LexemeType == E_BVH_LEXEME_TYPE_FLOAT || lexeme.LexemeType == E_BVH_LEXEME_TYPE_INTEGER)
                {
                    parsingStack.pop();
                    index++;
					...
					}else if (lexeme.LexemeType == E_BVH_LEXEME_TYPE_INTEGER)
					{
						bvhHierarchyNodeStack.top().Offset[currOffsetFilledIndex] = lexeme.getIntValue();
					}
            ...
            case E_BVH_TOKEN_T_FLOAT_FRAMETIME:
				...
				if (lexeme.LexemeType == E_BVH_LEXEME_TYPE_FLOAT || lexeme.LexemeType == E_BVH_LEXEME_TYPE_INTEGER)
                {
					...
					{
						frameLength = lexeme.getIntValue();
					}
                }
                else
                {
            ...
            case E_BVH_TOKEN_T_FLOAT_MOTIONDATA:
                ...
                if (lexeme.LexemeType == E_BVH_LEXEME_TYPE_FLOAT || lexeme.LexemeType == E_BVH_LEXEME_TYPE_INTEGER)
                {
					...
					{
						motionData[currMotionDataFilledIndex] = lexeme.getIntValue();
					}
