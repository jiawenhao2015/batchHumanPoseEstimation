...
namespace BVHViewer_DataAccess
{
    ...
    BvhLexer::BvhLexer()
    {
      ...
      //  mTransitionTable[E_BVH_STATE_BEGIN][E_BVH_CHAR_TYPE_POINT] = E_BVH_STATE_FAIL;
		mTransitionTable[E_BVH_STATE_BEGIN][E_BVH_CHAR_TYPE_POINT] = E_BVH_STATE_FLOAT;
    ...
    void BvhLexer::run(BvhLexingData *lexingData, const string &bvhStr)
    {
        ...
        for (int i = 0; i < bvhStrLength; i++)
        {
			...
			if (currState == E_BVH_STATE_FLOAT && ( (!bENotation && (ch == 'e' || ch == 'E')) || (bENotation && (ch == '+' || ch == '-') ) ) )
			{
				bENotation = true;
				newState = E_BVH_STATE_FLOAT;
			}else
			{
				newState = mTransitionTable[currState][charType];
